...
```{r}
records_broken_in_more_than_year <- select(filter(records, record_duration>365),record_duration)
records_broken_in_more_than_year
```
# Question 3
...
```{r}
records_longest_time_to_be_broken <- select(filter(records, time==max(time)), player, track, date, record_duration)
records_longest_time_to_be_broken
#records %>% arrange(desc(record_duration))%>% select(player,track,date,record_duration) %>% head(1) (-2)
```
...
```{r}
records %>% group_by(track) %>% summarize(mean_time=mean(time))
#The track with the fastest average record time is Banshee Boardwalk
...
```{r}
records <- records %>% mutate(records, year=substr(date, 1,4))
records
```
...
```{r}
records %>% group_by(year) %>% summarize(mean_time=mean(time)) %>% arrange(desc(mean_time))
```
# Question 7
...
```{r, fig.width=10}
library(ggplot2)
records %>% group_by(year) %>% summarize(mean_time=mean(time)) %>% arrange(desc(mean_time)) %>% ggplot(aes(year,mean_time)) + geom_bar(stat='identity')

#There is no trend cross years. There is no clear pattern in time.
```
...
```{r}
records %>% group_by(type) %>% ggplot(.,aes(time, fill=type)) + geom_histogram(position="identity", color="black", alpha=.5) + labs(x="Time",y="Frequency")

#The Single Lab have higher frequency in the lower time, and when the time goes higher, Three Lap have more frequency.
```
...
```{r}
records <- records %>% mutate(time_per=ifelse(type=="Three Lap", time/3, time))
records %>% group_by(type) %>% ggplot(.,aes(time_per, fill=type)) + geom_histogram(position="identity", color="black", alpha=.5) + labs(x="Time", y="Frequency")
```
# Question 10
...
```{r}
records %>% filter(shortcut=="No") %>% group_by(type) %>% ggplot(.,aes(time_per, fill=type)) + geom_histogram(position="identity", color="black", alpha=.5) + labs(y="Frequency")
```
...
```{r}
records %>% group_by(system_played) %>% summarize(mean_time=mean(time))

#There is a difference across the two game systems. It does affect performance of records set since NTSC have shorter time than PAL.
```
