...
```{r}
Mammals_Milk_Cluster<-hclust(dist(Mammals_Milk_Std))
plot(Mammals_Milk_Cluster,lwd=3,sub="",hang=-1,xlab="Mammals",cex.lab=1.25,main="Mammals clustered by Std. Milk Composition \n Euclidean Metric, Complete Linkage",frame.plot=T)
```
...
MMilk_Agg_K3<-Mammals_Milk %>%
  group_by(km_3) %>%
  summarize(Count=n(),Mean_Water=mean(Water),Mean_Protein=mean(Protein), Mean_Fat=mean(Fat),Mean_Lactose=mean(Lactose),Mean_Ash=mean(Ash)) %>% round(2)
MMilk_Agg_K3

MMilk_Agg_K4<-Mammals_Milk %>%
  group_by(km_4) %>%
  summarize(Count=n(),Mean_Water=mean(Water),Mean_Protein=mean(Protein),
            Mean_Fat=mean(Fat),Mean_Lactose=mean(Lactose),Mean_Ash=mean(Ash)) %>% round (2)
MMilk_Agg_K4
...
plot(c(1:8),wgss,type="b",pch=16,cex=1.4,ylim=c(0,150),xlab="Number of Groups", ylab="WGSS")
#Elbow plot suggest 2 to 3 clusters.
#Elbow plot shows the clusters by WGSS starting to slow down, which is hard to clearfy, where as Dendrogram shows clustering through nested hierarchy picture, which could be clearly identified how many clusters are there.
