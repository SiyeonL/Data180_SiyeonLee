...
Vans93 <- dplyr::select(filter(Cars93, Type=="Van"), Model, Price, EngineSize, Horsepower, MPG.highway, Length)
Vans93
nrow(Vans93)

#There are 9 objects 'Vans93' have
...
```{r}
#Use the `dist` and `as.matrix` functions to find the distance matrix for the vans set using the Euclidean metric.
Dist_Vans93 <- dist(Vans93[,-1])
Dist_Vans93_mat <- as.matrix(Dist_Vans93)

#converting it to a matrix and storing it as `Vans_Euclidean`.
Vans_Euclidean <- matrix(Dist_Vans93_mat, byrow=T, nrow=9)

#set the row and column names of `Vans_Euclidean` to the Models of the vans using the rownames and colnames functions
colnames(Vans_Euclidean)<-Vans93[,1]
rownames(Vans_Euclidean)<-Vans93[,1]

#round the values in this matrix to the hundredths place (two decimal points).
round(Vans_Euclidean, digits=2)

max(Vans_Euclidean)

#The distance between Aerostar and Caravan is the shortest
#The distance between Lumina_Apv and Eurovan is the longest
...
```{r}
#Use the `dist` and `as.matrix` functions to find the distance matrix for the vans set using the Manhattan metric.
Dist_Vans93 <- dist(Vans93[,-1], method="manhattan")
Dist_Vans93_mat <- as.matrix(Dist_Vans93)

#converting it to a matrix and storing it as `Vans_Manhattann`.
Vans_Manhattan <- matrix(Dist_Vans93_mat, byrow=T, nrow=9)

#set the row and column names of `Vans_Manhattan` to the Models of the vans using the rownames and colnames functions
...
#The distance between Quest and MPV is the shortest
#The distance between Eurovan and Lumina_Apv is the longest
```
...
a) Nearest-neighbor
min{d(1,2), d(1,3), d(1,5), d(4,2), d(4,5)} = min{1, 9, 1.6, 6, 5, 2} = 1


b) Farthest-neighbor
max{d(1,2), d(1,3), d(1,5), d(4,2), d(4,5)} = max{1, 9, 1.6, 6, 5, 2} = 9

c) Average-neighbor
(1/6)*(1 + 9 + 1.6 + 6 + 5 + 2) = 4.1
...
```{r}
vertebrate <- read.csv("/Users/siyeonlee/Desktop/Data180_SiyeonLee/hw5/vertebrate.csv")
nrow(vertebrate)

# Dataframe have 15 rows
vertebrate
```
...
plot(hclust(dist(Ver_Euclidean),method="complete"),lwd=3,sub='',xlab="Vertebrates",cex.lab=1.25,hang=-1,main="Clustered vertebrates, Euclidean, Complete Linkage")
```
...
plot(hclust(dist(Ver_Manhattan),method="single"),lwd=3,sub='',xlab="Vertebrates",cex.lab=1.25,hang=-1,main="Clustered vertebrates, Manhattan, Single Linkage")
```

Which method do you prefer?

# I prefer Euclidean method
...
ggdendrogram(hclust(dist(Ver_Euclidean),method="complete"),lwd=3,sub="",xlab="Vertebrates",cex.lab=1.25,hang=-1)
