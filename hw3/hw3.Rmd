...
```{r}
ggplot() + geom_point(aes(x=EngineSize, y=MPG.city),Cars93,col="blue", size=2) + labs(title="MPG in City and Engine Size",subtitle="Scatterplot based on Cars93 data", y="MPG.city", x="Engine Size", caption="MPG in City and Engine Size based on Cars93 data") + xlim(1,6)+ylim(15,50)
```
...
```{r}
ggplot() + geom_point(aes(x=EngineSize, y=MPG.city, col=Type),Cars93, size=2) + labs(title="MPG in City and Engine Size",subtitle="Scatterplot based on Cars93 data", y="MPG.city", x="Engine Size", caption="MPG in City and Engine Size based on Cars93 data") + xlim(1,6)+ylim(15,50)
...
```{r}
ggplot(Cars93, aes(x=EngineSize, y=MPG.city, col=Type)) + geom_point(size=2) +geom_smooth(color="red", linetype="dashed") + labs(title="MPG in City and Engine Size",subtitle="Scatterplot based on Cars93 data", y="MPG.city", x="Engine Size", caption="MPG in City and Engine Size based on Cars93 data") + xlim(1,6) +ylim(15,50)
```
...
```{r}
ggplot(Cars93, aes(AirBags, fill=AirBags)) + geom_bar() + labs(title = "Frequency of using AirBags", subtitle="based on Cars93 data", x="Usage of AirBags", y="Frequency", caption="Frequency of using AirBags based on Cars93 data")
```
...
```{r}
ggplot(Cars93, aes(x=Origin, fill=AirBags)) + geom_bar(position='dodge') + labs(title = "Count of AirBags Cars by Origin", subtitle="barplot",x="Origin",y="Frequency", caption="Count of AirBags Cars of each Driver or Passenger by Origin")
```
...
```{r}
ggplot() + geom_sf(data=au_states, aes(fill=NAME))
```
## Task 3
...
```{r}
ggplot() + geom_sf(data=au_states) + geom_point(aes(x=lon, y=lat, size=2), col="red", data=au_capitals)
```
...
```{r}
ggplot() + geom_sf(data=au_states) + coord_sf(crs = st_crs(3112))
```
...
```{r}
mapview(au_states) + mapview(capitals_geo, Legend=F, col.regions='red')
```
