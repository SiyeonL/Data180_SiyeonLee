...
```{r}
barplot(table(Cars93$Type),ylab="Count",xlab="Type",col=1:6,main="Frequency of Car93",cex.axis=1.2,cex.names=1.2)
```
...
```{r}
pie(table(Cars93$Type),main="Frequency of Car93",cex.axis=1.2,cex.names=1.2)
```
...
```{r}
hist((Cars93$Horsepower),xlab="Horsepower",ylab="Count",col="green",main="Histogram of Cars93 Horsepower",cex=1.2)
```
...
```{r}
hist((Cars93$Horsepower),xlab="Horsepower",ylab="Count",col="green",main="Histogram of Cars93 Horsepower",cex=1.2,breaks=c(50,75,100,125,150,175,200,225,250,275,300))
```
...
2nd histogram is better since it gives me 9.6 and 2nd one is closer than 1st one.
```{r}
sqrt(nrow(Cars93))
```
...
Numerical variables and it is discrete.
When we make a histogram for "passengers," it shows the range of the passengers of each frequency. The problem is since it is discrete variables, it is hard to determine exact number of each variables.
...
```{r}
barplot(table(Cars93$Passengers),ylab="Count",xlab="Passengers",col=1:6,main="Passengers of Car93",cex.axis=1.2,cex.names=1.2)
...
```{r}
hist(Cars93$Length,xlab="Length",ylab="Count",col="pink",main="Histogram of Cars93 Length",cex=1.2)
```
...
Yes, mean and median numbers are similar, it is just a small decimos differences.
```{r}
mean(Cars93$Length)
